...
TtmlFileSource::TtmlFileSource(const std::string& path) :
        PlainFileSource(path)
{
    // noop
}

TtmlFileSource::~TtmlFileSource()
{
    // noop
}

bool TtmlFileSource::readPacket(DataPacket& packet)
{
    ...
    {
        std::cerr << "Cannot stat file '" << getPath() << "'" << std::endl;
        return false;
    }
    ...
    const std::uint32_t type = PACKET_TYPE_TTML_DATA;
    const std::uint32_t size = packetDataSize + 4 /* for channelId */ + 8;
