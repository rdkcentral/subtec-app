...
CPPUNIT_TEST_SUITE( DocumentInstanceTest );
    ...
    CPPUNIT_TEST(testStartElementMultipleTTWarns);
    ...
    CPPUNIT_TEST(testGenerateTimelineImagesAndTextOverlapEmptyLines);
...
public:
    ...
    void testStartElementMultipleTTWarns()
    {
        DocumentInstance doc;
        doc.startElement("tt");
        auto tt2 = doc.startElement("tt");
        // Should log a warning, but we can't check logs here. Just ensure only one TTElement is created.
        CPPUNIT_ASSERT(tt2 == doc.startElement("tt"));
    }
    ...
    void testGenerateTimelineImagesAndTextOverlapEmptyLines()
    {
        DocumentInstance doc;
        doc.startElement("tt");
        auto img = doc.startElement("image");
        doc.endElement();
        auto p = doc.startElement("p");
        p->parseAttribute("", "begin", "00:00:00");
        p->parseAttribute("", "end", "00:00:10");
        p->appendText("text");
        doc.endElement();
        doc.endElement();
        auto timeline = doc.generateTimeline();
        ...
        doc.endElement();
        doc.endElement();
        auto timeline2 = doc.generateTimeline();
        CPPUNIT_ASSERT(timeline2.size() >= 0);
    }
