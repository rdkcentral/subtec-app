...
void sendccattribute(EdgeType type, std::string filename, AttribPosInArray attribute = EDGE_TYPE_ARR_POSITION )
{
    ...
    attributesValues[attribute] = type;

    attributesMask |= (1 << EDGE_COLOR_ARR_POSITION);
    attributesValues[EDGE_COLOR_ARR_POSITION] = COLOR_RED;
...
{
    debug_log("Test webvtt attributes edge type started");
    bool ret = false;
    ...
    if (ret)
    {
        ...
        sendccattribute(EDGE_TYPE_EMBEDDED, "input_files/test_edgeType_embedded.vtt");

        sleep (8);
        sendccattribute(EDGE_TYPE_NONE, "input_files/test_edgeType_none.vtt");

        sleep (8);
        sendccattribute(EDGE_TYPE_RAISED, "input_files/test_edgeType_raised.vtt");

        sleep (6);
        sendccattribute(EDGE_TYPE_DEPRESSED, "input_files/test_edgeType_depressed.vtt");

        sleep (6);
        sendccattribute(EDGE_TYPE_UNIFORM, "input_files/test_edgeType_uniform.vtt");

        sleep (6);
        sendccattribute(EDGE_TYPE_SHADOW_LEFT, "input_files/test_edgeType_left.vtt");

        sleep (6);
        sendccattribute(EDGE_TYPE_SHADOW_RIGHT, "input_files/test_edgeType_right.vtt");
