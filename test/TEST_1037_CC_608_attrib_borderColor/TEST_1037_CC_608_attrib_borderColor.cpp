...
{
    debug_log("Test CC 608 attribute window border color started");
    bool ret = false;
    ...
    if (ret)
    {
        ...
        while (offset + 24 <= hexData.length()) {
            ...
            if (counter == 0) {
                ...
                debug_log ("Border Type : Uniform");

                debug_log ("Border color : Black");
                sendCCAttributePacket(GSW_CC_ATTRIB_FONT_SIZE, GSW_CC_FONT_SIZE_EXTRALARGE, channel);
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_TYPE, GSW_CC_BORDER_TYPE_UNIFORM, channel);
                
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x00000000, channel); // CCColor_BLACK
            }
            if (counter == 4) {
                debug_log ("-------------------------");
                debug_log ("Border color : White");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x00ffffff, channel); // CCColor_WHITE
            }
            if (counter == 8) {
                debug_log ("-------------------------");
                debug_log ("Border color : Red");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x00FF0000, channel); // CCColor_RED
            }
            if (counter == 12) {
                debug_log ("-------------------------");
                debug_log ("Border color : Green");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x0000FF00, channel); // CCColor_GREEN
            }
            if (counter == 16) {
                debug_log ("-------------------------");
                debug_log ("Border color : Blue");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x000000FF, channel); // CCColor_BLUE
            }
            if (counter == 20) {
                debug_log ("-------------------------");
                debug_log ("Border color : Yellow");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x00FFFF00, channel); // CCColor_YELLOW
            }
            if (counter == 24) {
                debug_log ("-------------------------");
                debug_log ("Border color : Magenta");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x00FF00FF, channel); // CCColor_MAGENTA
            }
            if (counter == 28) {
                debug_log ("-------------------------");
                debug_log ("Border color : Cyan");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x0000FFFF, channel); // CCColor_CYAN
            }
