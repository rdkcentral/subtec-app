...
{
    debug_log("Test CC 608 attribute font edge type started");
    bool ret = false;
    ...
    if (ret)
    {
        ...
        while (offset + 24 <= hexData.length()) {
            ...
            if (counter == 4) {
                ...
                debug_log ("Edge Type : Shadow Right");
                sendCCAttributePacket(GSW_CC_ATTRIB_EDGE_COLOR, (int)0x00ffffff, channel); // CCColor_WHITE
                sendCCAttributePacket(GSW_CC_ATTRIB_EDGE_TYPE, GSW_CC_EDGE_TYPE_SHADOW_RIGHT, channel);
            }
            if (counter == 8) {
                debug_log ("-------------------------");
                debug_log ("Edge color : Green");
                debug_log ("Edge Type : Depressed");
                sendCCAttributePacket(GSW_CC_ATTRIB_EDGE_COLOR, (int)0x0000FF00, channel); // CCColor_GREEN
                sendCCAttributePacket(GSW_CC_ATTRIB_EDGE_TYPE, GSW_CC_EDGE_TYPE_DEPRESSED, channel);
            }
            if (counter == 12) {
                debug_log ("-------------------------");
                debug_log ("Edge color : Magenta");
                debug_log ("Edge Type : Uniform");
                sendCCAttributePacket(GSW_CC_ATTRIB_EDGE_COLOR, (int)0x00FF00FF, channel); // CCColor_MAGENTA
                sendCCAttributePacket(GSW_CC_ATTRIB_EDGE_TYPE, GSW_CC_EDGE_TYPE_UNIFORM, channel);
            }
            if (counter == 16) {
                debug_log ("-------------------------");
                debug_log ("Edge color : Yellow");
                debug_log ("Edge Type : Raised");
                sendCCAttributePacket(GSW_CC_ATTRIB_EDGE_COLOR, (int)0x00FFFF00, channel); // CCColor_YELLOW
                sendCCAttributePacket(GSW_CC_ATTRIB_EDGE_TYPE, GSW_CC_EDGE_TYPE_RAISED, channel);
            }
