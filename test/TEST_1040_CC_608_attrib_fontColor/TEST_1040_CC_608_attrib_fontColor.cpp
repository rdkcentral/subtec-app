...
{
    debug_log("Test CC 608 attribute font color started");
    bool ret = false;
    ...
    if (ret)
    {
        ...
        while (offset + 24 <= hexData.length()) {
            ...
            if (counter == 0) {
                ...
                sendCCAttributePacket(GSW_CC_ATTRIB_FONT_COLOR, (int)0x00000000, channel); // CCColor_BLACK
            }
            if (counter == 4) {
                debug_log ("-------------------------");
            ...
            if (counter == 12) {
                debug_log ("-------------------------");
                debug_log ("Font color : Green");
                sendCCAttributePacket(GSW_CC_ATTRIB_FONT_COLOR, (int)0x0000FF00, channel); // CCColor_GREEN
            }
            if (counter == 16) {
                debug_log ("-------------------------");
                debug_log ("Font color : Blue");
                sendCCAttributePacket(GSW_CC_ATTRIB_FONT_COLOR, (int)0x000000FF, channel); // CCColor_BLUE
            }
            if (counter == 20) {
                debug_log ("-------------------------");
                debug_log ("Font color : Yellow");
                sendCCAttributePacket(GSW_CC_ATTRIB_FONT_COLOR, (int)0x00FFFF00, channel); // CCColor_YELLOW
            }
            if (counter == 24) {
                debug_log ("-------------------------");
                debug_log ("Font color : Magenta");
                sendCCAttributePacket(GSW_CC_ATTRIB_FONT_COLOR, (int)0x00FF00FF, channel); // CCColor_MAGENTA
            }
            if (counter == 28) {
                debug_log ("-------------------------");
                debug_log ("Font color : Cyan");
                sendCCAttributePacket(GSW_CC_ATTRIB_FONT_COLOR, (int)0x0000FFFF, channel); // CCColor_CYAN
            }
