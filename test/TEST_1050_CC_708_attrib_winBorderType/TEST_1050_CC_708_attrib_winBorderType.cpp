...
{
    debug_log("Test CC 708 attribute window border type started");
    bool ret = false;
    ...
    if (ret)
    {
        ...
        while (offset + 24 <= hexData.length()) {
            ...
            if (counter == 0) {
                ...
                debug_log ("Border Type : Shadow Left");
                ...
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_TYPE, GSW_CC_BORDER_TYPE_SHADOW_LEFT, channel);
            }
            if (counter == 96) {
                debug_log ("-------------------------");
                debug_log ("Border color : Red");
                debug_log ("Border Type : Shadow Right");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x00FF0000, channel); // CCColor_RED
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_TYPE, GSW_CC_BORDER_TYPE_SHADOW_RIGHT, channel);
            }
            if (counter == 116) {
                debug_log ("-------------------------");
                debug_log ("Border color : Blue");
                debug_log ("Border Type : Depressed");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x000000FF, channel); // CCColor_BLUE
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_TYPE, GSW_CC_BORDER_TYPE_DEPRESSED, channel);
            }
            if (counter == 132) {
                debug_log ("-------------------------");
                debug_log ("Border color : Red");
            ...
            if (counter == 148) {
                debug_log ("-------------------------");
                debug_log ("Border color : Green");
                debug_log ("Border Type : Raised");
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_COLOR, (int)0x0000FF00, channel); // CCColor_GREEN
                sendCCAttributePacket(GSW_CC_ATTRIB_BORDER_TYPE, GSW_CC_BORDER_TYPE_RAISED, channel);
            }
