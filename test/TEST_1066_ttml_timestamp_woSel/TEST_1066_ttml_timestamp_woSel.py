...
# Get the directory of the current script - so that subtecConsole.log is created inside test folder
...
@pytest.mark.ttml
def test_1066():
    print("test_1066".ljust(50), "Test Started")
    ...
    wait_for_log_string(subtec_log_path, "onPacketReceived packet=type:TTML_TIMESTAMP")
    time.sleep(2)
    ...
    assert monitor_log_for_sequence(subtec_copy_path, log_sequence), "Logs for test_1066 missing"

    # Give strings that are not expected in log
    log_sequence = ['onPacketReceived packet=type:TTML_SELECTION',
        'currentMediatime mediatime=00:00:00.000']
    assert validate_string_absence(subtec_copy_path, log_sequence), "Logs for test_1066 incorrect"

    print("test_1066".ljust(50), "Test Passed")
