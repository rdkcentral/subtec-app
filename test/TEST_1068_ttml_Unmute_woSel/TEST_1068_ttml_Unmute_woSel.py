...
@pytest.mark.ttml
def test_1068():
    print("test_1068".ljust(50), "Test Started")
    ...
    wait_for_log_string(subtec_log_path, "onPacketReceived packet=type:UNMUTE")
    time.sleep(2)
    ...
    assert monitor_log_for_sequence(subtec_copy_path, log_sequence), "Logs for test_1068 missing"

    # Give strings that are not expected in log
    log_sequence = ['onPacketReceived packet=type:TTML_SELECTION',
        'unmute received',
        'setVisible visible=1']
    assert validate_string_absence(subtec_copy_path, log_sequence), "Logs for test_1068 incorrect"

    print("test_1068".ljust(50), "Test Passed")
