...
{
    debug_log("Test ttml attribute display align started");
    bool ret = false;
    ...
    if (ret)
    {
        ...
        read_file_send_data_packet_ttml("input_files/test_sample_displayAlign_before.ttml", channel);
        sleep (6);

        read_file_send_data_packet_ttml("input_files/test_sample_displayAlign_after.ttml", channel);
        sleep (6);

        read_file_send_data_packet_ttml("input_files/test_sample_displayAlign_center.ttml", channel);
    }
    else
    {
